...
def partition(my_list, start_idx, end_idx):
    ...
    for idx in range(start_idx, end_idx):
        ...
        if target <= pivot:
            ...
            my_list[partition_idx], my_list[idx] = \
                my_list[idx], my_list[partition_idx]
            
    # パーティションインデックスの位置を1つ進める
    partition_idx = partition_idx + 1

    # ピボットをパーティションインデックスの位置の要素と交換
    my_list[partition_idx], my_list[end_idx] = \
        my_list[end_idx], my_list[partition_idx]
